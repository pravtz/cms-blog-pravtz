...
export async function POST(
  ...
  if (user.role === 'owner') {
    return NextResponse.json({ error: 'Cannot suspend the owner account.' }, { status: 403 })
  }

  if (user.id === auth.payload.sub) {
  ...
  db.prepare("UPDATE users SET status = 'suspended', updated_at = datetime('now') WHERE id = ?").run(params.id)
